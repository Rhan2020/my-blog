...
解决上一篇文章中的HTTPS代理问题后，深夜的我本该满足地去睡觉，却忍不住开始审视整个项目结构 —— 天哪！我的项目里究竟积累了多少技术债务？
...
经过分析，我决定来一次彻底的"断舍离"，制定了以下简化原则：

- **专注核心功能**：CI/CD专注于Next.js应用部署
- **移除冗余**：将nginx配置回归服务器端管理
- **简化思维**：一个工具只做一件事
- **明确边界**：区分自动化与手动管理的界限
...
这种感觉就像Marie Kondo整理完衣柜的满足感 —— 每个文件都有明确的目的，每行代码都spark joy！
...
**总计**：删除289行代码，10个文件！这种轻量级的感觉真是太棒了。
...
而不是以前的：构建时担心破坏nginx配置，部署时盯着175行流程祈祷不要失败，调试时在应用层、代理层、容器层间痛苦跳转...
...
**核心理念**：不是所有功能都需要自动化，适度的手动管理有时更加高效。关键是在自动化的便利性和系统的复杂度之间找到合适的平衡点。 
